...
대각선이동은 막아놓은 알고리즘(가능하게 하고 싶으면 corner)
FindPath() 호출 뒤 xPath(), yPath() 로 다음 스텝의 위치를 얻어올 수 있음

배열의 x,y를 정방향으로 쓰는 점 주의!
...
#pragma once
	...
	//Declare constants
	static const int mapWidth = BOARD_COL, mapHeight = BOARD_ROW, tileSize = 1, numberPeople = MAX_NPC_SEED;
...
//Create needed arrays
	static char walkability [mapWidth][mapHeight];
	static int openList[mapWidth*mapHeight+2]; //1 dimensional array holding ID# of open list items
	static int whichList[mapWidth+1][mapHeight+1];  //2 dimensional array used to record 
// 	whether a cell is on the open list or on the closed list.
	static int openX[mapWidth*mapHeight+2]; //1d array stores the x location of an item on the open list
	static int openY[mapWidth*mapHeight+2]; //1d array stores the y location of an item on the open list
	static int parentX[mapWidth+1][mapHeight+1]; //2d array to store parent of each cell (x)
	static int parentY[mapWidth+1][mapHeight+1]; //2d array to store parent of each cell (y)
	static int Fcost[mapWidth*mapHeight+2];	//1d array to store F cost of a cell on the open list
	static int Gcost[mapWidth+1][mapHeight+1]; 	//2d array to store G cost for each cell.
	static int Hcost[mapWidth*mapHeight+2];	//1d array to store H cost of a cell on the open list
	static int pathLength[numberPeople+1];     //stores length of the found path for critter
	static int pathLocation[numberPeople+1];   //stores current position along the chosen path for critter		
	static int* pathBank [numberPeople+1];

	//Path reading variables
	static int pathStatus[numberPeople+1];
	static int xPath[numberPeople+1];
	static int yPath[numberPeople+1];

//-----------------------------------------------------------------------------
// Function Prototypes: where needed
//-----------------------------------------------------------------------------
static void ReadPath(int pathfinderID,int currentX,int currentY, int pixelsPerFrame);
static int ReadPathX(int pathfinderID,int pathLocation);
static int ReadPathY(int pathfinderID,int pathLocation);
...
//-----------------------------------------------------------------------------
static void InitializePathfinder (void)
{
...
//-----------------------------------------------------------------------------
static void EndPathfinder (void)
{
	for (int x = 0; x < numberPeople+1; x++)
	{
		free (pathBank [x]);
...
//-----------------------------------------------------------------------------
static int FindPath (int pathfinderID,int startingX, int startingY,
...
//	목적지는 갈 수 없는 곳이라고 해도 체크한다.
	if (walkability [a][b] != unwalkable || (a==targetX && b == targetY)) { 
...
//	Don't cut across corners
	...
	{
		// 대각선 이동 그 자체를 막음
		if (b == parentYval - 1 || b == parentYval + 1)
			corner = unwalkable;

		/*	장애물이 끼어 있는 대각선만 이동을 막음
		if (b == parentYval-1)
		{
